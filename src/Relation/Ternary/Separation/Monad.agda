...
module Monads
  {a b} {A : Set a}  {B : Set b} {{ra : RawSep A}} {u}
  {{as : HasUnit _≡_ ra u}}
  {{rb : RawSep B}}
  {{jm : Morphism A B}}
  {{bs : HasUnit _≡_ rb (Morphism.j jm u)}}
  where
  ...
  record Monad {i} (I : Set i) ℓ (M : RawMonad I ℓ) : Set (a ⊔ b ⊔ suc ℓ ⊔ i) where
    ...
    f =<< mp = bind (wand λ where σ px → coe (⊎-id⁻ˡ σ) (f px)) ⟨ ⊎-idˡ ⟩ mp
  ...
  module _ {i} {I : Set i} {i₁ i₂} {P} {M} {{ _ : Monad I a M }} where
    ...
    str mp σ qx = bind (wand λ where
      σ' px → case j-⊎⁻ σ' of λ where
        (_ , refl , σ'') → return (px ×⟨ ⊎-comm σ'' ⟩ qx)) ⟨ ⊎-comm σ ⟩ mp 
